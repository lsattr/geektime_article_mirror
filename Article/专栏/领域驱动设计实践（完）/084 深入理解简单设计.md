...
<meta content="084 深入理解简单设计" name="description"/>
<link href="/static/favicon.png" rel="icon"/>
<title>084 深入理解简单设计 </title>
...
<h1 class="title" data-id="084 深入理解简单设计" id="title">084 深入理解简单设计</h1>
<div><p>测试驱动开发遵守了测试—开发—重构的闭环。测试设定了新功能的需求期望，并为功能实现提供了保护；开发让实现真正落地，满足产品功能的期望；重构则是为了打磨代码质量，降低软件的维护成本。期望—实现—改进的螺旋上升态势，为测试驱动开发闭环提供了源源不断的动力。缺少任何一个环节，闭环都会停滞不动。没有期望，实现就失去了前进的目标；没有实现，期望就成为了空谈；没有改进，前进的道路就会越来越窄，突破就会变得愈发地艰难。</p>
<h3 id="重构改进的目标">重构改进的目标</h3>
