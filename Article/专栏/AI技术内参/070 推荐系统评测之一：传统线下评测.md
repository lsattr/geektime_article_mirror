...
<meta content="070 推荐系统评测之一：传统线下评测" name="description"/>
<link href="/static/favicon.png" rel="icon"/>
<title>070 推荐系统评测之一：传统线下评测 </title>
...
<h1 class="title" data-id="070 推荐系统评测之一：传统线下评测" id="title">070 推荐系统评测之一：传统线下评测</h1>
<div><p>上周我们讨论了EE算法，介绍了UCB（Upper Confidence Bound）算法和“汤普森采样”（Thompson Sampling）。</p>
...
<p>MSE是这么定义的。首先，如果我们知道一个用户i和物品j的真实评分，假设叫<span class="math inline">\( Y_{ij} \)</span> ，那么我们的一个估计值是<span class="math inline">\( Z_{ij} \)</span>，MSE计算的就是<span class="math inline">\( Y_{ij} \)</span> 和<span class="math inline">\( Z_{ij} \)</span>的差值，然后取平方。平方以后的结果肯定是一个正数，也就是说这样做的好处是整个计算不会出现负数，我们的估计值比真实值小了或者大了，MSE都可以处理。当我们对于每一个用户和物品都计算了这个差值以后，再对所有的差值的平方取一个平均值，就得到了我们所要的MSE。</p>
