...
<pre><code>public test_should_work() {
  ...
}
</code></pre>
...
<pre><code>assertEquals(2, calculator.add(1, 1));
</code></pre>
...
<pre><code>assertThat(calculator.subtract(4, 1), is(equalTo(3)));
</code></pre>
<p>AssertJ 是一种流畅风格的程序库，扩展性也非常不错，它也是我们在前面实战部分选择的程序库，下面是一段使用了 AssertJ 的代码。</p>
<pre><code>assertThat(frodo.getName()).startsWith("Fro")
                           .endsWith("do")
