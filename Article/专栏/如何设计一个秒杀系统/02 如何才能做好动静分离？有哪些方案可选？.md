...
<p><img alt="" src="assets/2c608715621afc9c95570dce00a87546.jpg"/></p>
<p>图1 静态化改造</p>
...
<h2 id="如何做动静分离的改造">如何做动静分离的改造</h2>
...
<ol>
<li><strong>URL唯一化</strong>。商品详情系统天然地就可以做到URL唯一化，比如每个商品都由ID来标识，那么<a href="http://item.xxx.com/item.htm?id=xxxx就可以作为唯一的URL标识。为啥要URL唯一呢？前面说了我们是要缓存整个HTTP连接，那么以什么作为Key呢？就以URL作为缓存的Key，例如以id=xxx这个格式进行区分。" target="_blank">http://item.xxx.com/item.htm?id=xxxx就可以作为唯一的URL标识。为啥要URL唯一呢？前面说了我们是要缓存整个HTTP连接，那么以什么作为Key呢？就以URL作为缓存的Key，例如以id=xxx这个格式进行区分。</a></li>
...
<h2 id="动静分离的几种架构方案">动静分离的几种架构方案</h2>
<p>前面我们通过改造把静态数据和动态数据做了分离，那么如何在系统架构上进一步对这些动态和静态数据重新组合，再完整地输出给用户呢？</p>
...
<p><img alt="" src="assets/c0fd22cf9d565a8ea2e9edcefae3b2dd.jpg"/></p>
<p>图4 CDN化部署方案</p>
